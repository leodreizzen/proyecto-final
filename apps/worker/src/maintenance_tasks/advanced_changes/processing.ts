...
export async function processAdvancedChange(changeId: string, targetResolutionId: string): Promise<{
    ...
    const otherChangesFiltered = otherChanges.filter(c=>c.resolvedForChangeAdvancedId !== changeId);
    ...
    const otherChangesForLLM = otherChangesFiltered;
    const modifierResolutionForLLM = mapResolutionForLLm(assembledModifierResolution.resolutionData);

    const hashInput = {
        articleContent: changeContentForLLM,
        targetResolution: targetResolutionForLLM,
        otherChanges: otherChangesForLLM,
        modifierResolution: modifierResolutionForLLM,
        modelVersion: MODEL_VERSION
    };
    const resolvedHash = crypto.createHash('sha256').update(stableStringify(hashInput)).digest('hex');

    if (changeData.resolvedHash === resolvedHash && changeData.resolvedAt && changeData.modelVersion === MODEL_VERSION) {
        console.log(`Skipping LLM analysis for change ${changeId} (hash match)`);
        return {
            success: true,
            changes: [],
            resolvedHash,
